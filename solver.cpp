...
void Solver::Solver::feasible_cal(const vector<size_t> &final_group, const size_t &k, double &w, double &w_r, double &h, double &h_u)
{
    ...
    for (size_t i = 0; i < final_group.size(); i++)
    {
        ...
        for (size_t kk = 0; kk < ptr_FF_D->faninCone.size(); kk++)
        {
            ...
            if (_FF_D_arr[FFid].grouped == false && _FF_D_arr[FFid].getClk() == k && renew == 1) // if the slack release to the grouped FF, what should i do ?
            {
    ...
    for (size_t i = 0; i < final_group.size(); i++)
    {
        ...
        for (size_t kk = 0; kk < ptr_FF_Q->fanoutCone.size(); kk++)
        {
            ...
            if (_FF_D_arr[FFid].grouped == false && _FF_D_arr[FFid].getClk() == k && renew == true) // if the slack release to the grouped FF, what should i do ?
            {
...
void Solver::Solver::solve_by_window(int &slice, double &low_weight, const string &filename)
{
    // low_weight = 0: put at the center of feasible
    // low_weight = 1; put at the lowest point of fesible
    // evaluate("init Metrix.txt");
    ...
    */
                ...
                while (!feas_x_clk.empty())
                {
                    ...
                    for (int i = 1; i < feas_x_clk.size(); i++)
                    {
                        ...
                        if ((feas_x_clk.at(i - 1).type) == 0 && (feas_x_clk.at(i).type == 1)) // 0 is start, 1 is end
                        {
                            ...
                            // consider the y part
                            pair<double, double> x_pos_r, y_pos_r; // final region, x_pos_r(s&e), y_pos_r(s&e)
                            vector<size_t> final_group;
                            vector<size_t> result_group;
                            // find the position, 還原成正常座標
                            ...
                            // preplace and slack release
                            pair<double, double> pos;                              // feasible center
                            ...
                            final_group = prePlace(result_group, esssential_ff, decision_pos);
                            // final_group = prePlace(result_group, esssential_ff, pos);
    ...
    // drawpic("pic_e.plt");
    string s1 = filename + "before_leg_Metrix.txt";
    evaluate(s1);
    string s2 = filename + "after_leg_Metrix.txt";
    legalize();
    evaluate(s2);
    // drawpic("pic_final.plt");
    std::cout << "Solver is completed !" << std::endl;
